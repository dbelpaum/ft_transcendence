...
const GameScene: React.FC<GameSceneProps> = ({ width, height, initialGameMode }) => {
	...
	useEffect(() => {
		if (canvasRef.current && gameMode === 'ai') {
			...
			mainLight.position.set(0, 0, 250);
			scene.add(mainLight);
			...
			const ballMaterial = new THREE.MeshLambertMaterial({ color: 0xc45c23 }); // THREE.MeshLambertMaterial({ color: 0xCC0000 });
